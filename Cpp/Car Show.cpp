...
int n, m, pointer = 1, arr[N], maxInd[N], vis[N*10], lazy[N<<2];
ll T[N<<2], ans[N];
vector <pii> E[N];
...
void push(int l, int r, int v){
	...
	T[v<<1] += (mid(l, r) - l + 1) * lazy[v];
	T[v<<1|1] += (r - mid(l, r)) * lazy[v];
...
int main(){
	...
	for(int i=1; i<=n; i++){
		while(pointer <= n && !vis[arr[pointer]])
			vis[arr[pointer++]]++;
	...
	for(int i=1; i<=m; i++){
		...
		E[left].pb(mp(right, i));
	}
	
	for(int i=n; i>=1; i--){
		upd(i, maxInd[i], 1, n, 1);
